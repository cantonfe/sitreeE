...
function (...) 
{
  ...
  if (vol.wo){
    ...
    vol.wo.bark.l[ i.tree.sp.2] <-
      SitkaVol(dbh   = dbh.mm[i.tree.sp.2],
    ...
    vol.wo.bark.l[i.tree.sp.4] <-
      LauvVol(tsl    = as.integer(tree.sp[i.tree.sp.4]),
  ...
  if (vol.w){
    ...
    ## GRAN
    vol.bark.l[i12] <- GranVol(dbh   = dbh.mm[i12], 
                               #f.dbh = dbh.mm[i12],
                               trh   = height.dm[i12],
                               bark  = 'mb',
                               enhet = 'l'
    ...
    ## FURU 
    vol.bark.l[i32]<- FuruVol(dbh   = dbh.mm[i32],
                              #f.dbh = dbh.mm[i32],
                              trh   = height.dm[i32],
                              bark  = 'mb',
                              enhet = 'l'
