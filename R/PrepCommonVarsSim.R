  ...
  if (any(vars.required %in% c("stand.age.years", "AgeTo5"))){
    ...
    if (i.period == 0){
      ...
      if (!all (tr$data$treeid %in% res$tree.age.years$treeid)){
        print("NEW TREES!!!!")
        i.new.trees <- tr$data$treeid[! tr$data$treeid %in% res$tree.age.years$treeid]
        rows.new.trees <- (
          (nrow(res$tree.age.years)+1) :
          (nrow(res$tree.age.years) +  length(i.new.trees))
          )
...
################################
  ...
  if(any(vars.required %in% c("time.since.final.felling"))){
    if(i.period==0){
      ...
      ## harvested between last period and this
      ...
      time.intern[!harv.last & !is.na(time.intern)] <-
        time.intern[!harv.last & !is.na(time.intern)] + period.length
    }
...
#######################
  ## HEIGHT
#######################
  ## Unusual variables for dbh-height Matthias models
  if (any(vars.required %in% c("xkti_Dg_22", "xkti_Dg"))){
    ## for spruce and pine
    res$xkti_Dg <- -1 * (
      ( (tr$data[["dbh.mm"]][, this.period]/10 + 20) ^ -2.0 - (30+20) ^ -2.0 ) /
      (                                    (10 + 20) ^ -2.0 - (30+20) ^ -2.0 )
    )
    ## for birch
    res$xkti_Dg[res$spp %in% c("birch", "other")] <- -1 * (
      ( (tr$data[["dbh.mm"]][res$spp %in% c("birch", "other"),
                             this.period]/10 + 20) ^ -2.0 - (30+18) ^ -2.0 ) /
      (                                    (10 + 20) ^ -2.0 - (30+18) ^ -2.0 )
    )
  }
...
################################
  ## BIOMASS kg
################################
  if (any(vars.required %in% c("biomass.tr.components.kg", "biomass.tr.kg"))){

    ## Calculate individual tree volume without bark
    res$biomass.tr.components.kg <- NULL
    res$biomass.tr.components.kg <- do.call(what = biomass.M1988,
                                            args = list (fn = "M1988.Rune",
                                                         tr = tr,
    ...
    if (any(vars.required %in% "biomass.tr.kg")) {
      res$biomass.tr.kg <- with(res$biomass.tr.components.kg,
                                living.branches.kg +
                                dead.branches.kg +
                                stem.wood.kg +
                                bark.kg +
                                (stump.roots.kg - stump.kg)
                                )
    }
...
################################################
  ## ADD THE MANAGEMENT INTENSITY CLASS STRATUM ##
  ## added by Nikolas 08.07.2015                ##
################################################
  if(any(vars.required %in% c("mic.stratum"))){
    lu.lt     <- fl$land.use %in% 1 & fl$land.type %in% 1
    is.pine   <- fl$SI.spp %in% 2
    is.spruce <- fl$SI.spp %in% 1
    is.hig    <- fl$SI.m >= 15.5
    is.med    <- fl$SI.m < 15.5 & fl$SI.m >= 10.5
    is.low    <- fl$SI.m < 10.5

    ## see below, they are the same!
    ## if a time-varying site-index should be used
    if(mng.options$si.time.varying){ 
      fl$mic.stratum <- rep("o", times = length(fl$ustandID))
      fl$mic.stratum[lu.lt & is.pine   & is.hig] <- "ph"
      fl$mic.stratum[lu.lt & is.pine   & is.med] <- "pm"
      fl$mic.stratum[lu.lt & is.pine   & is.low] <- "pl"
      fl$mic.stratum[lu.lt & is.spruce & is.hig] <- "sh"
      fl$mic.stratum[lu.lt & is.spruce & is.med] <- "sm"
      fl$mic.stratum[lu.lt & is.spruce & is.low] <- "sl" 
      ## if the site-index should be stable, the value of the first period is used
    } else if(!mng.options$si.time.varying & i.period==0) {
      fl$mic.stratum <- rep("o", times = length(fl$ustandID))
      fl$mic.stratum[lu.lt & is.pine   & is.hig] <- "ph"
      fl$mic.stratum[lu.lt & is.pine   & is.med] <- "pm"
      fl$mic.stratum[lu.lt & is.pine   & is.low] <- "pl"
